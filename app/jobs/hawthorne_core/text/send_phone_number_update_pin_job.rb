...
class HawthorneCore::Text::SendPhoneNumberUpdatePinJob < HawthorneCore::ApplicationJob
  ...
  def perform(user_id)
    ...
    if user_site.new_phone_number_pin_recently_sent?
      HawthorneCore::UserAction::Log.text_message_sent_failure(user_site.user_id, HawthorneCore::UserAction::FailureReason.text_message_recently_sent, { text_message_type: type, new_phone_number: new_phone_number, new_phone_number_pin: user_site.new_phone_number_pin })
      return
    end
