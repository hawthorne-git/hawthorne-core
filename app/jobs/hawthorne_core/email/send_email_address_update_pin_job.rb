...
class HawthorneCore::Email::SendEmailAddressUpdatePinJob < HawthorneCore::ApplicationJob
  ...
  def perform(user_id)
    ...
    if user_site.new_email_address_pin_recently_sent?
      HawthorneCore::UserAction::Log.email_sent_failure(user.id, HawthorneCore::UserAction::FailureReason.email_recently_sent, { type: type, new_email_address: user_site.new_email_address, new_email_address_pin: user_site.new_email_address_pin })
      return
    end

    # the pin was not recently sent, send the email
    HawthorneCore::Services::MailerSendSvc.send_email_address_update_pin(user.id, user.first_name, user_site.new_email_address, user_site.new_email_address_pin, user_site.new_email_address_pin_formatted)
