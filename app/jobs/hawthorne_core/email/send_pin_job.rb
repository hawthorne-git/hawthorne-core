...
class HawthorneCore::Email::SendPinJob < HawthorneCore::ApplicationJob
  ...
  def perform(site_user_id)
    ...
    unless site_user
      HawthorneCore::SiteUserAction::Log.email_sent_failure(nil, HawthorneCore::SiteUserAction::FailureReason.unexpected_state, { type: HawthorneCore::Services::MailerSendSvc.verification_pin, message: 'Site User not found', site_user_id: site_user_id })
      return
    end

    # exit unless the pin is active
    unless site_user.pin_active?
       HawthorneCore::SiteUserAction::Log.email_sent_failure(site_user.id, HawthorneCore::SiteUserAction::FailureReason.pin_expired, { type: HawthorneCore::Services::MailerSendSvc.verification_pin, pin: site_user.pin, pin_created_at: site_user.pin_created_at })
      return
    end
