...
class HawthorneCore::Email::SendEmailUpdateCodeJob < HawthorneCore::ApplicationJob
  ...
  def perform(user_id:)
    ...
    if user_site.new_email_code_recently_sent?
      HawthorneCore::UserAction::Log.email_sent_failure(user_id: user.id, failure_reason: HawthorneCore::UserAction::FailureReason.email_recently_sent, note: { type: type, new_email: user_site.new_email, new_email_code: user_site.new_email_code })
      return
    end
