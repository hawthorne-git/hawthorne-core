...
class HawthorneCore::UserController < HawthorneCore::ApplicationController
  ...
  def show
    ...
    HawthorneCore::ActiveRecordBase.connected_to(role: connect_to_role) do

      # get the user
      @user = Core::User.
        select(:email_address, :full_name, :phone_number).
        find_by(site_user_id: session[:user_id], site_id: Core::Site.this_site_shared_ids)

      # get all the users active credit cards
      @credit_cards = Core::UserCreditCard.
        select(:site_user_credit_card_id, :braintree_id).
        where(site_user_id: session[:user_id]).
        where(deleted: false).
