...
class HawthorneCore::User::SessionController < HawthorneCore::ApplicationController
  ...
  def sign_in
    ...
    HawthorneCore::Email::SendSignInCodeJob.perform_later(user_id: user.id, keep_signed_in:) if user.sign_in_code_default_delivery_via_email?
    HawthorneCore::Text::SendSignInCodeJob.perform_later(user_id: user.id) if user.sign_in_code_default_delivery_via_phone?

    return if true

    # redirect the user to verify their sign-in code
    redirect_to verify_sign_in_code_path(token: user.token, code_delivery_method: user.sign_in_code_default_delivery, keep_signed_in:)
  ...
  def verify_sign_in_code_show
    ...
    unless @user_token.present?
      HawthorneCore::UserAction::Log.sign_in_code_verified_failure(nil, HawthorneCore::UserAction::FailureReason.unexpected_state, { controller_action: 'verify_code_show', message: 'Token not present' })
    ...
    unless @user
      HawthorneCore::UserAction::Log.sign_in_code_verified_failure(nil, HawthorneCore::UserAction::FailureReason.unexpected_state, { controller_action: 'verify_code_show', message: 'User not found with token', token: @user_token })
      redirect_to sign_in_path and return
    end
  ...
  def verify_sign_in_code
    ...
    # in the unexpected case where the users token is not present, return back to the sign-in page
    unless user_token.present?
      HawthorneCore::UserAction::Log.sign_in_code_verified_failure(nil, HawthorneCore::UserAction::FailureReason.unexpected_state, { controller_action: 'verify_code', message: 'Token not present' })
      redirect_to sign_in_path and return
    end
    ...
    unless user
      HawthorneCore::UserAction::Log.sign_in_code_verified_failure(nil, HawthorneCore::UserAction::FailureReason.unexpected_state, { controller_action: 'verify_code', message: 'Site User not found with token', token: user_token })
      redirect_to sign_in_path and return
    end
    ...
    unless HawthorneCore::User::sign_in_code_delivery_methods.include?(code_delivery_method)
      HawthorneCore::UserAction::Log.sign_in_code_verified_failure(user.id, HawthorneCore::UserAction::FailureReason.unexpected_state, { controller_action: 'verify_code', message: 'Unexpected code delivery method', code_delivery_method: code_delivery_method })
      redirect_to sign_in_path and return
    end
    ...
    unless user_site.sign_in_code_active?
      ...
      HawthorneCore::UserAction::Log.sign_in_code_verified_failure(user.id, failure_reason, { sign_in_code: user_site.sign_in_code, sign_in_code_created_at: user_site.sign_in_code_created_at, sign_in_code_failed_attempts_count: user_site.sign_in_code_failed_attempts_count })
    ...
    unless user_site.sign_in_code_match?(code:)
      HawthorneCore::UserAction::Log.sign_in_code_verified_failure(user.id, HawthorneCore::UserAction::FailureReason.code_not_match, { code:, code_to_match: user_site.sign_in_code })
