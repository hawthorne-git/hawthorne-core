...
class HawthorneCore::User::Profile::PhoneNumberController < HawthorneCore::AccountApplicationController
  ...
  def render_code_not_match_error(user_site:, code:)
    HawthorneCore::UserAction::Log.update_profile_failure(failure_reason: HawthorneCore::UserAction::FailureReason.code_not_match, note: { action: 'UPDATE_PHONE_NUMBER', code:, code_to_match: user_site.new_phone_number_code })
