class HawthorneCore::ImageComponent < ViewComponent::Base
  ...
  def srcset
    ...
    widths.map { |width| "#{media_url_for_key(@image.file_key, version: @image.file_version, width: width)} #{width}w" }.join(', ')
  end
  ...
  def src
    return media_url_for_key(@image.file_key, version: @image.file_version) if widths.empty?
    media_url_for_key(@image.file_key, version: @image.file_version, width: widths.max)
  end
